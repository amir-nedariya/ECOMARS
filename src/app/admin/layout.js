...
import {
    LayoutDashboard, PackageSearch, Tags, ShoppingCart, Users, Ticket, LineChart, Settings,
    LogOut,
    Menu,
    X
} from "lucide-react";
 ...
 `}>
                <div className="p-6 flex items-center justify-between border-b border-slate-800">
                    <Link href="/admin" className="flex items-center gap-3">
                        <Image src="/logo.png" alt="Ecomars Logo" width={56} height={56} className="object-contain" priority />
                        <span className="text-indigo-500 text-lg tracking-wide font-medium">ADMIN PANEL</span>
                    </Link>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.name} href={item.href}
                                className={`
 flex items-center gap-3 px-5 py-3.5 transition-all relative z-10 ${isActive
