...
export default function ProductManagementPage() {
  const [products, setProducts] = useState([
    { id: 1, name: "Premium Blue Perfume", category: "Perfume", price: "₹2,499", stock: 45, status: "Active", image: "https://images.unsplash.com/photo-1594035910387-fea47794261f?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 2, name: "Stainless Steel Water Bottle", category: "Bottle", price: "₹899", stock: 120, status: "Active", image: "https://images.unsplash.com/photo-1602143407151-7111542de6e8?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 3, name: "Clear Storage Plastic Box", category: "Plastic", price: "₹450", stock: 0, status: "Out of Stock", image: "https://images.unsplash.com/photo-1584016221727-463d1a8e6ea4?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 4, name: "Designer Black Perfume", category: "Perfume", price: "₹3,199", stock: 12, status: "Low Stock", image: "https://images.unsplash.com/photo-1594035910387-fea47794261f?auto=format&fit=crop&q=80&w=100&h=100" },
  ]);
  ...
  const getCategoryBadge = (category) => {
    switch(category) {
      ...
      case 'Bottle': return 'bg-green-100 text-green-600';
      case 'Plastic': return 'bg-orange-100 text-orange-600';
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link href="/admin/products/add" className="bg-[#1F2336] hover:bg-slate-800 text-white px-6 py-2.5 transition-all flex items-center gap-2 rounded-md">
      ...
      {/* New Table matching the Image Look */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden rounded-lg shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#1F2336] text-white text-xs font-semibold tracking-wide">
                <th className="p-4 pl-6 whitespace-nowrap">Image</th>
            ...
            <tbody className="text-sm">
              {products.map((product) => (
                <tr key={product.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 text-xs font-semibold rounded-full whitespace-nowrap ${getCategoryBadge(product.category)}`}>
                      {product.category}
                    </span>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-center gap-3">
                      <Link href={`/admin/products/edit/${product.id}`} className="text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors" title="Edit">
                        <Edit2 className="h-4 w-4" />
                      </Link>
                      <button onClick={() => deleteProduct(product.id)} className="text-red-500 hover:text-red-700 transition-colors" title="Delete">
        ...
        <div className="flex items-center justify-end px-6 py-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-4 text-sm text-slate-500 font-medium">
            <span>1 to {products.length} of {products.length}</span>
